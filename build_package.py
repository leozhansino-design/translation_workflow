...
def clean_build_dirs():
    ...
    # 清理.spec文件
    spec_files = [
        'OutlineGenerator.spec',
        'NovelWriter.spec',
        'WriterTaskManager.spec',
        'TaskManager.spec'
    ]
...
def build():
    ...
    # 主程序入口
    entry_points = [
        ('outline_generator.py', 'OutlineGenerator', '大纲生成器'),
        ('writer_app.py', 'NovelWriter', '小说生成器'),
    ]

    for script, name, display_name in entry_points:
        print(f"\n{'='*60}")
        print(f"正在构建: {display_name} ({name})")
        print(f"{'='*60}")
    ...
    print(f"\n使用方法:")
    if system == 'Windows':
        print(f"  Tool 1 (大纲生成器): dist\\OutlineGenerator.exe")
        print(f"  Tool 2 (小说生成器): dist\\NovelWriter.exe")
    else:
        print(f"  Tool 1 (大纲生成器): ./dist/OutlineGenerator")
        print(f"  Tool 2 (小说生成器): ./dist/NovelWriter")

    print(f"\n工作流程:")
    print(f"  1. 运行 OutlineGenerator.exe 生成大纲")
    print(f"  2. 运行 NovelWriter.exe 生成小说内容")
